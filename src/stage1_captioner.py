"""
Stage 1: 数据拓源与基础特征提取（Gemini / OpenAI 兼容 API）。

职责：读音频 → 调用远端多模态 API → 产出 Base Caption JSON
（transcript、speaker_profile、core_intent 等）。
"""
...
from utils.metadata_contract import normalize_meta_data, safe_string
...
DEFAULT_API_KEY = os.environ.get("AFCHAT_API_KEY", "")
DEFAULT_MODEL_NAME = "gemini-3-flash-preview"
DEFAULT_PROXY = os.environ.get("AFCHAT_HTTP_PROXY", "")
...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Stage 1: Generate base captions from audio clips using remote multimodal API."
    )
    ...
    parser.add_argument("--proxy", default=DEFAULT_PROXY, help="Optional HTTP(S) proxy.")
    parser.add_argument(
        "--prompt-mode",
    ...
    parser.add_argument(
        ...
        action="store_true",
        help="Backfill transcript/emotion hints from the source manifest.",
    )
...
def get_system_prompt(prompt_mode: str):
    return EMOTION_AWARE_SYSTEM_PROMPT if prompt_mode == "emotion-aware" else EMOTION_AWARE_SYSTEM_PROMPT
...
def run_stage1_batch(*args, **kwargs):
    """批量处理 data/01_raw_audio → 写入 data/02_base_captions"""
    main()
...
def main():
    ...
    print(f"=== Stage 1 Caption 任务启动: {len(tasks)} 条 | 模型: {args.model} ===")
    print(
